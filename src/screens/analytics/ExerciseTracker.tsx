...
import {
  RPE_TABLE_MODE_TO_E1RM_FUNCTION,
  RPE_TABLE_MODES,
  RpeTableMode,
} from "../RPEChart";

function getGroupId() {
  return `${Date.now().toString(16)}-${randomInt(1, 10_000)}`;
}
 ...
 */
function extractDataToGraph(
  settings: GroupSettings,
  workouts: FullAttachedWorkout<"log">[],
): Map<ISODate, Map<UUID, DataToGraph>> {
  ...
  workouts.forEach((workout) => {
    ...
    workout.exercises.forEach((exercise, i) => {
      ...
      if (settings.volume.filterWarmups) {
        setsForThisExercise = setsForThisExercise.filter(
          (s) => s.set_type !== "warmup",
        );
      }
    ...
    if (dataForExercisesInThisWorkout.size > 0) {
      if (finalMapping.has(workout.workout.completed_on)) {
        const dataOnDateAlready = finalMapping.get(
          workout.workout.completed_on,
        )!;
        dataForExercisesInThisWorkout.forEach((newDataToGraph, exId) => {
          if (dataOnDateAlready.has(exId)) {
            ...
            const combined: DataToGraph = {
              bestE1RM: maxNullable(
                oldDataToGraph.bestE1RM,
                newDataToGraph.bestE1RM,
              ),
              numberOfSets:
                oldDataToGraph.numberOfSets + newDataToGraph.numberOfSets,
              rpesLogged: [
                ...oldDataToGraph.rpesLogged,
                ...newDataToGraph.rpesLogged,
              ],
            };
...
function averageInPlace(
  data: { value: number }[],
  averagingStrategy:
    | { strat: "sliding"; windowSize: number }
    | { strat: "rolling"; alpha: number }
    | null,
): void {
  if (data.length <= 1) {
    return;
  ...
  if (averagingStrategy === null) {
    ...
    if (ws <= 0) {
      console.error(
        `Got a window size of ${ws} which is not allowed. Default to ws=7`,
      );
      ws = 7;
    }
    ...
    if (alpha < 0 || alpha > 1) {
      console.error(
        `Got alpha=${alpha} but has constraints 0<=alpha<=1. Setting alpha=0.80`,
      );
      alpha = 0.8;
    }
...
function addInterpolationInPlace(
  ...
  for (let k = 0; k < knownIdx.length - 1; k++) {
    ...
    if (numDaysBetweenDates <= 0) {
      console.error(
        `Days should be increasing. Got date1=${base[i0].date}, date2=${base[i1].date}`,
      );
      continue;
    }
    ...
    for (let i = i0 + 1; i < i1; i++) {
      const frac =
        daysBetweenDates(base[i0].date, base[i].date) / numDaysBetweenDates;
...
function buildE1RMSeries(
  ...
  }[] = dateRangeFromMinToMax.map((date) => {
    const exMap = allDates.get(date);
  ...
  notKnown.forEach(
    (i) => (base[i] = { ...base[i], dataPointColor: "transparent" }),
  );
  return base.map((v) => {
    return {
      label: v.date,
      value: v.value === null ? undefined : v.value,
      dataPointColor: v.dataPointColor,
    };
  });
}
  ...
  switch (group.trackingMetric) {
    case "sets":
      return buildVolumeSeries(
        allDates,
        dateRangeFromMinToMax,
        group.volume.averagingStrategy === null
          ? null
          : {
              strat: group.volume.averagingStrategy,
              windowSize: group.volume.windowSize,
              alpha: group.volume.alpha,
            },
      );
    case "e1rm":
      return buildE1RMSeries(allDates, dateRangeFromMinToMax);
    case "rpe":
      return buildAverageRPESeries(allDates, dateRangeFromMinToMax);
    default:
      throw new Error(`Unknown metricL ${group.trackingMetric}`);
  }
}
...
function reduceWorkoutsForGroup<M extends WorkoutEditorMode>(
  trackOverWorkouts: FullAttachedWorkout<M>[],
  group: GroupSettings,
): FullAttachedWorkout<M>[] {
  return trackOverWorkouts
    .filter((w) =>
      w.exercises.some((ex) => group.exercises.has(ex.exercise.id)),
    )
    .map((w) => {
      const fitleredWithIndex = w.exercises
        .map((ex, i) => [ex, i] satisfies [EditableExercise<M>, number])
        .filter(([ex, _i]) => group.exercises.has(ex.exercise.id));
...
export function ExerciseTracker<M extends WorkoutEditorMode>(
  props: ExerciseTrackerProps<M>,
) {
  const { trackOverWorkouts } = props;

  // exercise picker
  const [exerciseModalVisible, setExerciseModalVisible] =
    useState<boolean>(false);
  const [exerciseModalFn, setExerciseModalFn] = useState<
    ((exercise: ExerciseRow) => void) | null
  >(null);
  const [exerciseModalIdLimiter, setExerciseModalIdLimiter] = useState<
    Set<UUID>
  >(new Set());

  // all groups
  const [seeAllExercises, setSeeAllExercises] = useState<boolean>(false);
  const [filterAllExercisesByTags, setFilterAllExercisesByTags] = useState<
    ExerciseAndMuscleTag[]
  >([]);
  ...
  const [confirmDeleteAllGroups, setConfirmDeleteAllGroups] =
    useState<boolean>(false);
  ...
  const allExerciseInWorkout: Map<UUID, ExerciseRow> = useMemo(() => {
    const newMap = new Map();
    trackOverWorkouts.forEach((w) =>
      w.exercises.forEach((e) => newMap.set(e.exercise.id, e.exercise)),
    );
    return newMap;
  }, [trackOverWorkouts]);
  const allExerciseIdsInWorkouts = useMemo(() => {
    const ids = new Set<UUID>();
    for (const w of trackOverWorkouts) {
      for (const e of w.exercises) ids.add(e.exercise.id);
    }
    return [...ids];
  }, [trackOverWorkouts]);

  const allExerciseIdsSig = useMemo(
    () => allExerciseIdsInWorkouts.slice().sort().join("|"),
    [allExerciseIdsInWorkouts],
  );
  ...
  function addAllExercisesWithTagsToGroup(
    groupIndex: number,
  ...
  function removeAllExercisesWithTagsToGroup(
    groupIndex: number,
    tags: ExerciseAndMuscleTag[],
  ) {
        ...
        >
          <Button
            ...
            onPress={() => {
              ...
              setExerciseModalIdLimiter(
                new Set(
                  allExerciseIdsInWorkouts.filter(
                    (exId) => !group.exercises.has(exId),
                  ),
                ),
              );
          ...
          <Text style={typography.hint}>
            (
            {
              allExerciseIdsInWorkouts.filter((id) => !group.exercises.has(id))
                .length
            }{" "}
            exercises left to add)
          </Text>
        </View>
        {group.exercises.size > 0 ? (
          [...group.exercises.values()].map((ex) => {
            return (
              <View key={`${ex.id}-${group.id}`} style={styles.exercisePill}>
                <Text>{ex.name}</Text>
                <Feather
                  name={"x"}
                  onPress={() => removeExercisesFromGroup(groupIndex, [ex])}
                />
              </View>
            );
          })
          ...
          <Text style={{ ...typography.label, marginLeft: spacing.sm }}>
            No exercises in Group {groupIndex + 1} yet
          </Text>
        )}
      </View>
    );
  ...
  const renderGroupDropdown = (group: GroupSettings, groupIndex: number) => {
        ...
        <Feather
          ...
          onPress={() =>
            updateGroup(groupIndex, {
              showAdvancedSettings: !group.showAdvancedSettings,
            })
          }
        />
      </View>
      ...
      <View style={{ ...styles.containerStyle, alignSelf: "flex-start" }}>
        ...
        >
          <Selection
            ...
            onPress={() =>
              updateGroupVolume(groupIndex, {
                filterWarmups: !volume.filterWarmups,
              })
            }
            ...
            {trackOverWorkouts
              .map((w) =>
                w.sets
                  .map((s) =>
                    s
                      .map((ss) => (ss.set_type === "warmup" ? 1 : 0) as number)
                      .reduce((a, b) => a + b, 0),
                  )
                  .reduce((a, b) => a + b, 0),
              )
              .reduce((a, b) => a + b, 0)}{" "}
            Sets of Warmups Exist
          </Text>
        </View>
        <View
          style={{
        ...
        >
          ...
          <ModalPicker
            ...
            onChange={(value) =>
              updateGroup(groupIndex, { e1RMFormula: value })
            }
            pressableProps={{
        ...
        >
          <ModalPicker
            options={[null, "rolling", "sliding"].map((v) => {
              return {
                label:
                  v === null
                    ? "No Averaging"
          ...
          {volume.averagingStrategy === "rolling" && (
            <>
              <Feather
                ...
                onPress={() => {
                  updateGroupVolume(groupIndex, {
                    alpha: Math.max(0, volume.alpha - 0.05),
                  });
                }}
              />
              ...
              <Feather
                ...
                onPress={() => {
                  updateGroupVolume(groupIndex, {
                    alpha: Math.min(1, volume.alpha + 0.05),
                  });
                }}
              />
            </>
          ...
          {volume.averagingStrategy === "sliding" && (
            <>
              <Feather
                ...
                onPress={() => {
                  updateGroupVolume(groupIndex, {
                    windowSize: Math.max(1, volume.windowSize - 1),
                  });
                }}
              />
              <Text style={typography.body}>{volume.windowSize}</Text>
              <Feather
                name="plus-circle"
                size={22}
                onPress={() => {
                  updateGroupVolume(groupIndex, {
                    windowSize: Math.min(100, volume.windowSize + 1),
                  });
                }}
              />
            </>
        ...
        >
          ...
          {METRICS.map((m) => (
            <Selection
              key={m}
              title={metricLabel(m)}
              isSelected={group.trackingMetric === m}
              onPress={() => updateGroup(groupIndex, { trackingMetric: m })}
            />
          ))}
        </View>
      </View>
    ...
    const dropdownExercises = (
      ...
      >
        ...
        <Feather
          ...
          onPress={() =>
            updateGroup(groupIndex, {
              showExercisesInGroup: !group.showExercisesInGroup,
            })
          }
        />
      </View>
    ...
    const dropdownQuickAdd = (
      ...
      >
        ...
        <Feather
          name={group.showQickAdd ? "arrow-up-circle" : "arrow-down-circle"}
          size={18}
          onPress={() =>
            updateGroup(groupIndex, { showQickAdd: !group.showQickAdd })
          }
        />
      </View>
    );
      ...
      <View style={styles.containerStyle}>
        ...
        >
          <Button
            ...
            {...styles.smallButton}
            onPress={() =>
              addAllExercisesWithTagsToGroup(groupIndex, group.tagsToLookAt)
            }
          />
          <Button
            title="Remove All"
            {...styles.smallButton}
            onPress={() =>
              removeAllExercisesWithTagsToGroup(groupIndex, group.tagsToLookAt)
            }
          />
              ...
              <Selection
                ...
                onPress={() => {
                  if (group.tagsToLookAt.includes(t)) {
                    updateGroup(groupIndex, {
                      tagsToLookAt: group.tagsToLookAt.filter((ot) => ot !== t),
                    });
                  } else {
                    updateGroup(groupIndex, {
                      tagsToLookAt: [...group.tagsToLookAt, t],
                    });
                  }
                }}
                {...styles.smallButton}
        ...
        <Text style={typography.hint}>
          {
            new Set(
              trackOverWorkouts.flatMap((w) =>
                w.exercises
                  .filter((e) =>
                    isSubsetOfArray(group.tagsToLookAt, e.exercise.tags),
                  )
                  .map((e) => e.exercise.id),
              ),
            ).size
          }{" "}
          exercises have all selected tags
        </Text>
      </View>
    );
    return (
  ...
  const renderFullGroupMetadata = (
    group: GroupSettings,
    groupIndex: number,
  ) => {
    return (
      <View style={{ marginBottom: spacing.sm }} key={group.id}>
        <View
          style={{
            ...styles.containerStyle,
            flexDirection: "row",
        ...
        >
          ...
          <Feather
            name={group.displayGroup ? "arrow-up-circle" : "arrow-down-circle"}
            size={22}
            onPress={() =>
              updateGroup(groupIndex, { displayGroup: !group.displayGroup })
            }
          />
          <Feather
            style={{ ...typography.body, marginLeft: "auto" }}
            name="copy"
            size={22}
            onPress={() => copyGroup(groupIndex)}
          />
          <Feather
            style={{ ...typography.body, marginLeft: spacing.xs }}
            name="trash"
            size={22}
            onPress={() => setConfirmDeleteGroup(groupIndex)}
          />
        </View>
        {group.displayGroup && (
          <View
  ...
  const renderAllGroupsGraphData = () => {
    ...
    if (groups.length === 0) {
      return (
        <Text style={typography.hint}>Add groups to track exercise(s)</Text>
      );
    }
    if (!groups.some((g) => g.exercises.size > 0)) {
      return (
        <Text style={typography.hint}>
          Add exercises to groups for tracking
        </Text>
      );
    }
    ...
    if (minDate === null || maxDate === null) {
      console.error(
        `Got minDate=${minDate} and maxDate=${maxDate}. Neither should be null`,
      );
      return (
    ...
    for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
      ...
      if (allDatesData.size === 0) {
        dataSet.push({
          data: [],
          color: groupColor,
          dataPointsColor: groupColor,
        });
        continue;
      }
      let graphData = buildSeriesForGroupMetric({
        group,
        allDates: allDatesData,
        dateRangeFromMinToMax,
      });
      if (applyNormalization) {
        ...
        graphData = graphData.map((p) => {
          ...
          return {
            ...p,
            value: minMaxNormalize(p.value, minMax?.min ?? 0, minMax?.max ?? 0),
          };
        });
      }
      dataSet.push({
    ...
    const legend = (
      <View style={{ ...styles.containerStyle, flexWrap: "wrap" }}>
        <View
          style={{
            ...styles.containerStyle,
        ...
        >
          <Selection
            title={"Normalize"}
            isSelected={applyNormalization}
            onPress={() => setApplyNormalization((prev) => !prev)}
          />
          <Selection
            title={"One Graph"}
            isSelected={oneGraph}
            onPress={() => setOneGraph((prev) => !prev)}
          />
        </View>
    ...
    if (oneGraph) {
      ...
      const min =
        applyNormalization || atLeastOneNotE1RM
          ? 0
          : (computeMinMax(
              dataSet
                .map((d) => computeMinMax(d.data)?.min ?? 0)
                .map((n) => {
                  return { value: n };
                }),
            )?.min ?? 0);
        ...
        <View style={{ gap: spacing.sm }}>
          ...
          {dataSet.map((data, i) => {
            if (data.data.length === 0) {
              return (
                <Text key={`${groups[i].id}`} style={typography.body}>
                  Nothing to graph for Group {i + 1}
                </Text>
              );
            }
            const min =
              applyNormalization || groups[i].trackingMetric === "e1rm"
                ? (computeMinMax(data.data)?.min ?? 0)
                : 0;
            return (
              <LineChart
                key={`${groups[i].id}`}
  ...
  const renderWorkoutsMetadata = () => {
    const allExerciseInWorkoutFiltered = [
      ...allExerciseInWorkout.values(),
    ].filter((e) => isSubsetOfArray(filterAllExercisesByTags, e.tags));
    return (
      <View>
        <View
          style={{
        ...
        >
          <Text style={{ ...typography.body }}>
            Tracking over {trackOverWorkouts.length} workout
            {trackOverWorkouts.length === 1 ? "" : "s"}
          </Text>
          <Text style={{ ...typography.body, marginLeft: "auto" }}>
            See All Exercises
          </Text>
          <Feather
            name={seeAllExercises ? "arrow-up-circle" : "arrow-down-circle"}
            size={18}
          ...
          >
                  ...
                  <Selection
                    ...
                    onPress={() => {
                      if (filterAllExercisesByTags.includes(t)) {
                        setFilterAllExercisesByTags((prev) =>
            ...
            {groups.length > 0 && (
              <Selection
                ...
                style={{ alignSelf: "flex-start" }}
                textProps={{
                  style: { ...typography.body, padding: spacing.padding_sm },
                }}
              />
            )}
          </View>
        ...
        {seeAllExercises &&
          (allExerciseInWorkoutFiltered.length > 0 ? (
            allExerciseInWorkoutFiltered.map((ex) => {
              return (
                <View
                  key={ex.id}
                  style={{
                ...
                >
                  ...
                  {showGroupsToAddTo && groups.length > 0 && (
  ...
  return (
    <View>
      ...
      >
        ...
        >
          Delete Group{" "}
          {confirmDeleteGroup !== null ? confirmDeleteGroup + 1 : "n/a"}?
        </Text>
        <Button
          ...
          variant="revert"
        />
        <Button title="Cancel" onPress={() => setConfirmDeleteGroup(null)} />
      </ClosableModal>
      <ClosableModal
        visible={confirmDeleteAllGroups}
        onRequestClose={() => setConfirmDeleteAllGroups(false)}
      >
        <Text
          style={{
            ...typography.title,
